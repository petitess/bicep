...
module data 'modules/data.bicep' = {
  ...
  params: {
    ...
    workspaceResourceId: resourceId(subscription().subscriptionId, rgMonitor.name, 'Microsoft.OperationalInsights/workspaces', 'log-${prefixMonitor}-01')
...
resource rgAvail 'Microsoft.Resources/resourceGroups@2023-07-01' = [for avail in filter(config.vm, v => !empty(v.availabilitySetName) && contains(v.name, '01')): {
  name: toLower('rg-${avail.name}')
  location: location
  tags: config.tags
}]

module avail 'modules/avail.bicep' = [for (avail, i) in filter(config.vm, v => !empty(v.availabilitySetName) && contains(v.name, '01')): {
  scope: rgAvail[i]
...
resource rgVm 'Microsoft.Resources/resourceGroups@2023-07-01' = [for vm in filter(config.vm, v => empty(v.availabilitySetName)): {
  name: toLower('rg-${vm.name}')
  location: location
  tags: config.tags
}]

module vm 'modules/vm.bicep' = [for (vm, i) in config.vm: {
...
output RgAvail array = [for avail in filter(config.vm, v => !empty(v.availabilitySetName) && contains(v.name, '01')): {
  name: avail.name
}]

output rgNoAvail array = [for vm in filter(config.vm, v => empty(v.availabilitySetName)): {
  name: vm.name
}]
