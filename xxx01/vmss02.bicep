...
var getMsbuild = 'powershell Invoke-WebRequest -Uri https://aka.ms/vs/17/release/vs_buildtools.exe -OutFile c:\\Tools\\vs_buildtools.exe'
var msbuild = 'powershell Start-Process -FilePath "c:\\Tools\\vs_buildtools.exe" -ArgumentList "--includeRecommended", "--quiet", "--wait", "\'--installPath c:\\VStools\'", "\'--add Microsoft.VisualStudio.Workload.ManagedDesktop\'" -Wait'
...
resource powershell 'Microsoft.Compute/virtualMachineScaleSets/extensions@2023-03-01' = if (imageReference.publisher == 'microsoftwindowsserver') {
  ...
  properties: {
    ...
    settings: {
      commandToExecute: '${folder};${getMsbuild};${msbuild};${GetAzureCliWin};${azureCliWin};${getPwshWin};${pwshWin}'
    }
  }
}
