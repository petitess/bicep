...
$vmname = 'vmmgmtprod01'
$localmachineScript = 'C:\B3\AdPasswordExpirationEntra.ps1'
...
Import-AzAutomationRunbook `
    -Name "run-ADpassExp01" `
    -ResourceGroupName $env:infrargname `
    -AutomationAccountName $env:aaname `
    -Tags @{Application = 'AD Password Expiration'} `
    -Type PowerShell `
    -Description "A script to execute a powershell script on vmmgmtprod01 - AdPasswordExpirationEntra.ps1, to check the expiration password date for user accounts. Triggered by Logic App." `
    -Published `
