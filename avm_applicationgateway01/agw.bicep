      ...
      firewallPolicy: !contains(site, 'waf')
        ...
        : {
            id: resourceId(
              ...
              'waf-${replace(prefix, 'waf', site.name)}-01'
            )
          }
    }
  }
...
var listenersHttps = [
  for (site, i) in !httpOnlyDeploy ? sites : []: {
    ...
    properties: {
      ...
      httpListener: {
        id: resourceId(
          ...
          name,
          'listener-${site.name}-http'
        )
      }
      redirectConfiguration: !httpOnlyDeploy
        ? {
            id: resourceId(
              subscription().subscriptionId,
              rg.name,
        ...
        : null
      backendHttpSettings: !contains(site, 'redirection') && httpOnlyDeploy
        ? {
            id: resourceId(
              subscription().subscriptionId,
              rg.name,
        ...
        : null
      backendAddressPool: !contains(site, 'redirection') && httpOnlyDeploy
        ? {
            id: resourceId(
              subscription().subscriptionId,
              rg.name,
...
var routingRulesHttps = [
  for (site, i) in !httpOnlyDeploy ? filter(sites, site => !contains(site, 'pathBased')) : []: {
      ...
      firewallPolicy: !contains(rule, 'waf')
        ...
        : {
            id: resourceId(
              ...
              rg.name,
              'Microsoft.Network/ApplicationGatewayWebApplicationFirewallPolicies',
              'waf-${replace(prefix, 'waf', rule.name)}-01'
            )
          }
    }
  }
...
var redirectsHttpToHttps = [
  for site in !httpOnlyDeploy ? sites : []: {
...
resource kv 'Microsoft.KeyVault/vaults@2024-04-01-preview' existing = if (!httpOnlyDeploy) {
...
module agw 'br:mcr.microsoft.com/bicep/avm/res/network/application-gateway:0.5.0' = {
  ...
  params: {
    ...
    sslCertificates: [
      for (sslCertificate, i) in !httpOnlyDeploy ? sslCertificates : []: {
        name: sslCertificate
        properties: {
