...
resource pepR 'Microsoft.Network/privateEndpoints@2025-01-01' = [
  for pep in privateEndpoints: {
    name: 'pep-${name}-${pep.key}'
    location: location
    properties: {
      customNetworkInterfaceName: 'nic-pep-${name}-${pep.key}'
      ipConfigurations: [
        {
          name: 'config-${pep.key}'
          properties: {
...
resource dns 'Microsoft.Network/privateEndpoints/privateDnsZoneGroups@2025-01-01' = [
...
output key1 string = st.listKeys().keys[0].value
