...
param runtimeName resourceInput<'Microsoft.Web/sites@2025-03-01'>.properties.functionAppConfig.runtime.name = 'dotnet-isolated'
param runtimeVersion resourceInput<'Microsoft.Web/sites@2025-03-01'>.properties.functionAppConfig.runtime.version = '8.0'
param slot ({
  name: ('stage')?
...
resource appSettingsWeb 'Microsoft.Web/sites/config@2025-03-01' = if (!isFlexConsumptionTier) {
  name: 'web'
  parent: func
  properties: {
...
resource authR 'Microsoft.Web/sites/config@2025-03-01' = {
  name: 'authsettingsV2'
  parent: func
  properties: {
    platform: {
...
resource funcSlot 'Microsoft.Web/sites/slots@2025-03-01' = if (!empty(slot) && !isFlexConsumptionTier) {
...
resource authSlotR 'Microsoft.Web/sites/slots/config@2025-03-01' = if (!empty(slot) && !isFlexConsumptionTier) {
  name: 'authsettingsV2'
  parent: funcSlot
  properties: {
    platform: {
...
resource hostNamedBindings 'Microsoft.Web/sites/hostNameBindings@2025-03-01' = if (!empty(customDomain)) {
...
resource hostNamedBindingsSlots 'Microsoft.Web/sites/slots/hostNameBindings@2025-03-01' = if (!empty(slot) && !empty(slot.?customDomain) && !isFlexConsumptionTier) {
...
resource pepR 'Microsoft.Network/privateEndpoints@2025-05-01' = [
  for pep in items(privateEndpoints): if (pep.key == 'sites' || pep.key == 'sites-stage' && !isFlexConsumptionTier) {
