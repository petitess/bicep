  ...
  properties: {
    ...
    subnets: [for (subnet, i) in subnets: {
      ...
      properties: {
        ...
        delegations: contains(subnet.properties, 'delegations') ? [
          {
            name: subnet.properties.delegations
            properties: {
              serviceName: subnet.properties.delegations
            }
          }
        ] : []
      }
...
resource nsg 'Microsoft.Network/networkSecurityGroups@2023-05-01' = [for subnet in subnets: if (subnet.properties.networkSecurityGroup) {
...
resource rt 'Microsoft.Network/routeTables@2023-05-01' = [for subnet in subnets: if (subnet.properties.routeTable) {
  name: 'rt-${subnet.name}'
  location: location
  tags: tags
...
resource ng 'Microsoft.Network/natGateways@2023-05-01' = if (natGateway) {
  name: 'ng-${name}'
  location: location
  tags: tags
  sku: {
...
resource pip 'Microsoft.Network/publicIPAddresses@2023-05-01' = if (natGateway) {
  name: 'pip-ng-${name}'
  location: location
  tags: tags
  sku: {
