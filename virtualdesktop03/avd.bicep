...
resource vdpool 'Microsoft.DesktopVirtualization/hostPools@2022-04-01-preview' = {
  name: '${name}01'
  location: location
  properties: {
    hostPoolType:  'Pooled'
...
resource applicationGroup 'Microsoft.DesktopVirtualization/applicationGroups@2022-04-01-preview' = {
  ...
  properties: {
    ...
    hostPoolArmPath: resourceId('Microsoft.DesktopVirtualization/hostpools', vdpool.name)
  }
}
...
output hpname string = vdpool.name
output appgrpname string = applicationGroup.name
output regkey string = vdpool.properties.registrationInfo.token
