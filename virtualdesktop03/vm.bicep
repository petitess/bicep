...
resource vm_WinAD 'Microsoft.Compute/virtualMachines/extensions@2022-03-01' = if(vm.name == 'vmdctest01') {
  parent:  vm
  name: 'WinAD'
  location: location
  tags: {
  ...
  properties: {
    ...
    settings: {
      fileUris: [
        'https://raw.githubusercontent.com/petitess/powershell/main/Install-WindowsFeature01.ps1'
      ]
      commandToExecute: 'powershell.exe -ExecutionPolicy bypass -File Install-WindowsFeature01.ps1'
    }
  }
}

resource virtualMachine_RSAT 'Microsoft.Compute/virtualMachines/extensions@2021-11-01' = if(vm.name == 'vmmgmttest01') {
  name: 'RSAT'
  location: location
  parent: vm
  properties: {
    ...
    typeHandlerVersion: '1.9'
    settings: {
      commandToExecute: 'powershell Install-WindowsFeature -IncludeAllSubFeature RSAT'
    }
  }
}
