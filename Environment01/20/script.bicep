...
var vma = [for vm in virtualMachinesAvail: {
  name: vm.name
}] 
...
$VirtualMachinesAvail = $env:VirtualMachinesAvail | ConvertFrom-Json -AsHashtable
    foreach ($Vm in $VirtualMachinesAvail) {
    $GetSecretPass = Get-AzKeyVaultSecret -VaultName $env:KeyVault -Name $Vm.name
    if ($null -eq $GetSecretPass) {
        $Bytes = New-Object Byte[] 24
        ([System.Security.Cryptography.RandomNumberGenerator]::Create()).GetBytes($Bytes)
