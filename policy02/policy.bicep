...
resource definition 'Microsoft.Authorization/policyDefinitions@2021-06-01' = [for policyDefinition in policyDefinitions: {
  name: 'AzurePolicyDefinition-${replace(subscription().displayName, ' ', '')}-${policyDefinition.name}'
  properties: {
    displayName: 'AzurePolicyDefinition-${replace(subscription().displayName, ' ', '')}-${policyDefinition.name}'
...
resource initiative 'Microsoft.Authorization/policySetDefinitions@2021-06-01' = {
  name: 'AzurePolicyInitiative-${replace(subscription().displayName, ' ', '')}-${policyInitiative.name}'
  properties: {
    displayName: 'AzurePolicyInitiative-${replace(subscription().displayName, ' ', '')}-${policyInitiative.name}'
    ...
    policyDefinitions: [
      {
        policyDefinitionId: '${subscription().id}/providers/Microsoft.Authorization/policyDefinitions/${definition[0].name}'
        policyDefinitionReferenceId: definition[0].name
        parameters: {}
      }
      {
        policyDefinitionId: '${subscription().id}/providers/Microsoft.Authorization/policyDefinitions/${definition[1].name}'
        policyDefinitionReferenceId: definition[1].name
        parameters: {}
      }
      {
        policyDefinitionId: '${subscription().id}/providers/Microsoft.Authorization/policyDefinitions/${definition[2].name}'
      ...
      {
        policyDefinitionId: '${subscription().id}/providers/Microsoft.Authorization/policyDefinitions/${definition[3].name}'
...
resource assignment 'Microsoft.Authorization/policyAssignments@2022-06-01' = {
  name: substring('AzurePolicyAssignment-${replace(subscription().displayName, ' ', '')}-${policyAssignment.name}', 0, 23)
  location: location
  properties: {
    displayName: 'AzurePolicyAssignment-${replace(subscription().displayName, ' ', '')}-${policyAssignment.name}'
    description: policyAssignment.description
    notScopes: policyExclusions
    enforcementMode: policyAssignment.enforcementMode
    policyDefinitionId: '${subscription().id}/providers/Microsoft.Authorization/policySetDefinitions/${initiative.name}'
